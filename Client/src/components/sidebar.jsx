...
function Sidebar() {
  const [isBedSpaceDropdownOpen, setIsBedSpaceDropdownOpen] = useState(false);
  const [isReportDropdownOpen, setIsReportDropdownOpen] = useState(false);

  const toggleBedSpaceDropdown = () => {
    setIsBedSpaceDropdownOpen(prevState => !prevState);
    isReportDropdownOpen === true &&
      setIsReportDropdownOpen(prevState => !prevState);
  };

  const toggleReportDropdown = () => {
    setIsReportDropdownOpen(prevState => !prevState);
    setIsBedSpaceDropdownOpen === true &&
      setIsBedSpaceDropdownOpen(prevState => !prevState);
  };
  ...
  return (
    <div className="fixed flex flex-col bg-white border-r h-screen w-[350px] pt-5 z-40">
      ...
      {/* <Scrollbar noDefaultStyles disableTracksWidthCompensation style={{ position: "" }}> */}
        <ul id="" className="innerSidebar h-fit overflow-y-scroll scroll-smooth pb-10">
          <li className="">
            <NavLink
              to="/"
          ...
          <li>
            <button
              onClick={toggleBedSpaceDropdown}
              className={
